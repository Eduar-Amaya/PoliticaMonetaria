##################### Paquetes requeridos #########################

# install.packages("ggplot2")
# install.packages("ggfortify")
# install.packages("tseries")
# install.packages("forecast")
# install.packages("vars")

#################################################
############# Librerias requeridas #######################

library(tseries)
library(ggfortify)
library(forecast)
library(vars)

# En los modelos VAR anteriores se han utilizado datos de las tasas de interes
# (Activas y Pasivas) de los bancos comerciales. Para este modelo VAR se utilizaran
# Datos del sistema financiero hondureño.
# Ademas se ha dividido en dos periodos el primero que comprende de Mayo de 2005 a 
# Diciembre de 2012, el segundo de Enero de 2013 a Agosto de 2021. 
# Para este reporte se trabajara del Mes de Mayo 2005 a Diciembre de 2012

##################################### VAR #######################################
# Cargamos la base de datos para el primer periodo antes descrito

Datos <- read.csv("BancoCH.csv")

# Cargamos primero los datos de Tasa de interes activa 
# Y Luego creamos el objeto ts
TasaAct.ts <- ts(Datos$Tasa_Acti, start = c(2005,5), end = c(2012,12), frequency = 12)
TasaAct.ts

# Cargamos los datos de Tasa de Interes Pasiva y creamos el objeto ts
TasaPas.ts <- ts(Datos$Tasa_Pas, start = c(2005,5), end = c(2012,12), frequency = 12)
TasaPas.ts

# Cargamos los datos de Tasa de Politica Monetaria y creamos el objeto ts
TPM.ts <- ts(Datos$TPM, start = c(2005,5), end = c(2012,12), frequency = 12)
TPM.ts

# Cargamos los datos de Indice de Precios al Consumidor y creamos el objeto ts
IPC.ts <- ts(Datos$IPC, start = c(2005,5), end = c(2012,12), frequency = 12)
IPC.ts

# Graficamos las series en conjunto 
ts.plot(TasaAct.ts,TasaPas.ts,TPM.ts,IPC.ts, xlab = "Tiempo", ylab = "Datos",
        col = c("blue","brown","purple","orange"), main = "Representacion de las Series")
legend(2006, 250, legend=c("TIA","TIP","TPM", "IPC"),
       col=c("blue","brown","purple","orange"), lty=1:4, cex=0.8)

# Veamos si las series son estacionarias utilizando el test de Dickey-Fuller, \alpha = 5%
# H_{0}: La serie no es estacionaria. p > 0 .05
# H_{1}: La serie es estacionaria p < 0.05

# Para la Tasa de Interes Activa 
Adf <- adf.test(TasaAct.ts)
Adf$p.value
# La serie no es estacionaria. Ya que p = 0.2970746

# Para Tasa Interes Pasiva 
Pdf <- adf.test(TasaPas.ts)
Pdf$p.value
# La serie no es estacionaria. Ya que p = 0.6489775

# Para Tasa de Politica Monetaria
Mdf <- adf.test(TPM.ts)
Mdf$p.value
# La serie no es estacionaria. Ya que p = 0.3135323

# Para Indice de Precios al Consumidor
Idf <- adf.test(IPC.ts)
Idf$p.value
# La serie no es estacionaria. Ya que p = 0.4156172

##############3 Aplicamos Logaritmo ##########################
TasaAct_log <- log(TasaAct.ts)

TasaPas_log <- log(TasaPas.ts)

TPM_log <- log(TPM.ts)

IPC_log <- log(IPC.ts)

####################### Aplicamos una diferencia ##################
TasaAct_dif1 <- diff(TasaAct_log)
TasaPas_dif1 <- diff(TasaPas_log)
TPM_dif1 <- diff(TPM_log)
IPC_dif1 <- diff(IPC_log)

# Graficamos las series con una diferencia
ts.plot(TasaAct_dif1,TasaPas_dif1,TPM_dif1,IPC_dif1, xlab = "Tiempo", ylab = "Datos",
        col = c("blue","brown","purple","orange"), main = "Representacion de las Series con una Diferencia")
legend(2006, -0.1, legend=c("TIA","TIP","TPM", "IPC"),
       col=c("blue","brown","purple","orange"), lty=1:4, cex=0.8)

# Veamos si las series son estacionarias utilizando el test de Dickey-Fuller, \alpha = 5%
# Despues de haber aplicado una diferencia
# H_{0}: La serie no es estacionaria. p > 0 .05
# H_{1}: La serie es estacionaria p < 0.05

# Para la Tasa de Interes Activa 
Adf1 <- adf.test(TasaAct_dif1)
Adf1$p.value
# La serie no es estacionaria. Ya que p = 0.275591

# Para Tasa Interes Pasiva 
Pdf1 <- adf.test(TasaPas_dif1)
Pdf1$p.value
# La serie no es estacionaria. Ya que p = 0.2792266

# Para Tasa de Politica Monetaria
Mdf1 <- adf.test(TPM_dif1)
Mdf1$p.value
# La serie no es estacionaria. Ya que p = 0.05919946

# Para Indice de Precios al Consumidor
Idf1 <- adf.test(IPC_dif1)
Idf1$p.value
# La serie es estacionaria

######################33 Aplicando segunda diferencia ############################
TasaAct_dif2 <- diff(TasaAct_dif1)
TasaPas_dif2 <- diff(TasaPas_dif1)
TPM_dif2 <- diff(TPM_dif1)
IPC_dif2 <- diff(IPC_dif1)

# Graficamos las series con dos diferencias
ts.plot(TasaAct_dif2,TasaPas_dif2,TPM_dif2,IPC_dif2, xlab = "Tiempo", ylab = "Datos",
        col = c("blue","brown","purple","orange"), main = "Representacion de las Series con dos Diferencia")
legend(2006, 0.3, legend=c("TIA","TIP","TPM", "IPC"),
       col=c("blue","brown","purple","orange"), lty=1:4, cex=0.8)

# Veamos si las series son estacionarias utilizando el test de Dickey-Fuller, \alpha = 5%
# Despues de haber aplicado dos diferencias
# H_{0}: La serie no es estacionaria. p > 0 .05
# H_{1}: La serie es estacionaria p < 0.05

# Para la Tasa de Interes Activa 
Adf2 <- adf.test(TasaAct_dif2)
Adf2$p.value
# La serie es estacionaria. 

# Para Tasa Interes Pasiva 
Pdf2 <- adf.test(TasaPas_dif2)
Pdf2$p.value
# La serie es estacionaria. 

# Para Tasa de Politica Monetaria
Mdf2 <- adf.test(TPM_dif2)
Mdf2$p.value
# La serie es estacionaria. 

# Para Indice de Precios al Consumidor
Idf2 <- adf.test(IPC_dif2)
Idf2$p.value
# La serie es estacionaria

####################### Creando un nuevo data frame ###################3333
A <- TasaAct_dif2
P <- TasaPas_dif2
M <- TPM_dif2
I <- IPC_dif2
Datos_VAR <- data.frame(A,P,M,I)

# Identificacion de nuestro modelo VAR
VARselect(Datos_VAR, type = "const")

# Seleccionamos nuestro modelo con 4 retardos
Var3 <- VAR(Datos_VAR,p=4)
Var3
summary(Var3)

# Pruebas de Especificacion 
# Prueba de Autocorrelacion Serial en los Residuales
# H_{0}: Los Residuales no estan correlacionados, > 0.05
# H_{1}: Los residuales si estan correlacionados, p < 0.05
serial1 <- serial.test(Var3, lags.pt = 4, type = "PT.asymptotic")
serial1$serial
# Concluimos que si hay una correlacion serial

# ****************************************************#
# Prueba de Normalidad de los Residuales
# H_{0}: Los Residuales se distribuyen normal > 0.05
# H_{1}: Los residuales no se distribuyen normal < 0.05
NorVar1 <- normality.test(Var3)
NorVar1$jb.mul
# ****************************************************#

# Prueba de Homocedasticidad de la Varianza de los residuales
# H_{0}: La Varianza de los residuales es constante >0.05
# H_{1}: L Varianza de los residuales no es constante <0.05
arch1 <- arch.test(Var3, lags.multi = 12)
arch1$arch.mul
# Se concluye que la varianza es constante

########################## Causalidad de Granger ##########################
# La tasa de Politica Monetaria no causa a Tasa de Interes Activa
# La tasa de Politica Monetaria no causa al IPC
# La tasa de Politica Monetaria no causa a Tasa de Cambio
grangertest(P~M, order = 1)
# La tasa de Politica Monetaria si causa en el sentido de Granger a la TIP

grangertest(M~I, order = 1)
#El IPC si causa en el sentido de Granger a la TPM

grangertest(M~A, order = 2)
# La TIA si causa en el sentido de Granger a la TPM

grangertest(P~A, order = 4)
# La TIA si causa en el sentido de Granger a la TIP

grangertest(M~P, order = 3)
# La TIP si causa en el sentido de Granger a la TPM

# Funciones Impulso Respuestas

impulso <- irf(Var3)
impulso
plot(impulso)


